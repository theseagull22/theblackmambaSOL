...
app = FastAPI(title="TV Receiver + Bybit Demo Adapter v0")
...
class AdapterConfig:
    ...
    default_order_qty: str = "5"
    default_leverage: str = "50"
    ...
    timeout: int = 10
    journal_path: str = "/tmp/bybit_adapter_journal_v0.json"
    ...
    def from_env(cls) -> "AdapterConfig":
        ...
        cfg = cls(
            ...
            default_order_qty=os.getenv("DEFAULT_ORDER_QTY", "5").strip() or "5",
            default_leverage=os.getenv("DEFAULT_LEVERAGE", "50").strip() or "50",
            ...
            journal_path=os.getenv("ADAPTER_JOURNAL_PATH", "/tmp/bybit_adapter_journal_v0.json"),
...
class BybitDemoAdapterV0:
    ...
    @staticmethod
    def _is_leverage_not_modified_error(exc: Exception) -> bool:
        text = str(exc).lower()
        return "110043" in text or ("leverage" in text and "not modified" in text)

    @staticmethod
    def _is_missing_amend_error(exc: Exception) -> bool:
        text = str(exc).lower()
        return (
            "110001" in text
            or "order not exists or too late to replace" in text
            or ("order not exists" in text and "replace" in text)
            or "too late to replace" in text
        )

    def _rebuild_place_params_for_missing_amend(self, command: ExecutionCommand) -> Optional[Dict[str, Any]]:
        if not command.order_id:
            return None
        registry = self.journal.order_registry.get(command.order_id, {})
        desired_state = registry.get("desired_state") or {}
        side = desired_state.get("side")
        qty = desired_state.get("qty") or command.params.get("qty")
        price = desired_state.get("price") or command.params.get("price")
        take_profit = desired_state.get("takeProfit")
        stop_loss = desired_state.get("stopLoss")
        ...
        payload: Dict[str, Any] = {
            ...
            "price": price,
            "timeInForce": "GTC",
            "orderLinkId": command.params.get("orderLinkId") or registry.get("order_link_id"),
            "reduceOnly": False,
    ...
    def _ensure_symbol_leverage(self, tv_symbol: str, symbol: str) -> None:
        ...
        try:
            ...
            if not self._is_leverage_not_modified_error(exc):
                raise RuntimeError(f"set_leverage rejected for {symbol}: {exc}") from exc
            result = {
                "retCode": 110043,
        ...
        try:
            if command.kind == "place_pending":
                ...
                try:
                    ...
                    if not self._is_missing_amend_error(exc):
                        raise
                    place_params = self._rebuild_place_params_for_missing_amend(command)
                    if not place_params:
                        raise RuntimeError(
                            f"amend_pending fallback could not rebuild place params for {command.order_id}: {exc}"
                        ) from exc
                    if command.order_id:
                        registry = self.journal.order_registry.setdefault(command.order_id, {})
                        registry.update(
                            {
                                "status": "stale_missing_on_exchange",
                                "last_error": str(exc),
                                "updated_at": utc_now_iso(),
                            }
                        )
                    self.journal.append_action(
                        "amend_missing_fallback_place",
                        "Amend target missing on exchange; retrying as fresh place.",
                        {
                    ...
                    self._persist()
                    result = self.gateway.place_pending_order(place_params)
                    self._register_order_after_place(
                        ExecutionCommand(
                            command_id=command.command_id,
                            kind="place_pending",
                            order_id=command.order_id,
                    ...
                    executed_kind = "amend_missing_fallback_place"
            ...
            self.journal.append_action(
                ...
                {"command_id": command.command_id, "kind": executed_kind, "result": result},
            )
            self._persist()
            return record
            ...
            self.journal.append_action(
                ...
                {"command_id": command.command_id, "kind": executed_kind, "error": str(exc)},
            )
            self._persist()
            return record
...
def root() -> Dict[str, Any]:
    return {
        "status": "ok",
        "service": "tv_receiver_bybit_adapter_v0",
        "time": utc_now_iso(),
